...
#define BUFSIZE 512
//
void communication(const struct sockaddr *sa,socklen_t fromlen, char *host, int ns){
	int s;        
	ssize_t nread, nwrite;
	char buf[BUFSIZE];
	char *message = "Message a envoyer: ";
	s = getnameinfo(sa, fromlen, host, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
	if (s == 0)
...
}//end function communication

int main(int argc, char **argv) {
	int sfd, s, ns, r;
	struct addrinfo hints; //utilisé pour l'adresse du serveur
	struct addrinfo *result, *rp;//rp est pour l'adresse du client
	//char buf[BUFSIZE];
	//ssize_t nread, nwrite;
	...
	for (;;) {
		...
		switch(pid){
			case 0://on est dans le fils
				communication((struct sockaddr *)&from, fromlen, host, ns);
				break;
			case -1:
				printf("erreur ni fils ni pere");
				break;
