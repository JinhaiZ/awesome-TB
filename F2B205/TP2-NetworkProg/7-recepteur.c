...
#define EXAMPLE_GROUP "224.2.2.10"
...
int main(int argc, char **argv) {
	...
	struct ip_mreqn multicastArg;
	multicastArg.imr_multiaddr.s_addr = inet_addr(EXAMPLE_GROUP);
	multicastArg.imr_address.s_addr = htonl(INADDR_ANY);
	multicastArg.imr_ifindex = 0;




	/* connecte au groupe multicast */
	if (setsockopt(sfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &multicastArg, 
				sizeof(multicastArg)) == 0)
		printf("connecte a adresse multicast \n");
	else
		perror("echec de connexion au multicast \n");
